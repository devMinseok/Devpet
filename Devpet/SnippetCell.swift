...
class SnippetCell: NSTableCellView {
    ...
    func setData(snippet: Snippet) {
        self.snippet = snippet
        
        snippetNameLabel.stringValue = snippet.name
        languageLabel.stringValue = snippet.language
    }
}
...
extension SnippetCell: NSPasteboardItemDataProvider {
    func pasteboard(_ pasteboard: NSPasteboard?, item: NSPasteboardItem, provideDataForType type: NSPasteboard.PasteboardType) {
        if let pasteboard = pasteboard,type == .string {
            pasteboard.setString(self.snippet?.data ?? "", forType: type)
        }
    }
}
